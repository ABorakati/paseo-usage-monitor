...
| `codex`               | Subscription | Session, weekly, code-review, reserve, credits | `GET https://chatgpt.com/backend-api/wham/usage`                             | `token`            | `tokens.access_token` in `${CODEX_HOME}/auth.json`, then `~/.codex/auth.json`, then `~/.config/codex/auth.json`      |
