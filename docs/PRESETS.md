...
| `claude-statusline`             | Subscription | Session and weekly limits                                     | None — reads `~/.claude/paseo-rate-limits.json`                                  | —                  | Install the hook from Usage provider settings; nothing is authenticated                                                     |
...
- **`claude`** polls every 30 minutes because its endpoint throttles hard — see [Rate limits](#rate-limits). It also reports **paid extra usage**, the overage billed once the plan's included limits are spent — see [Extra usage](#extra-usage) for which of the endpoint's two overage views it reads and why it draws no ceiling. **`claude-statusline`** reads the same two windows from a local file within fifteen seconds of the turn that wrote it, with no credential and no request — see [Reading Claude quota without a token](CREDENTIALS.md#reading-claude-quota-without-a-token). It carries no extra-usage row: the CLI hands a statusline nothing but the two windows.
- **`codex`** shows **Use banked reset** only while Codex reports at least one saved reset. Pressing it fetches the current credit details, shows its expiry, and requires confirmation before spending it. A successful reset refreshes the five-hour and weekly windows. The plugin never applies one from polling, never retries the mutation automatically, and hides the action for API-key accounts. The action uses Codex's own `rate-limit-reset-credits` backend routes with the access token and account id from the same `auth.json`.
...
Every preset except `antigravity`, `github-copilot`, `deepseek-rate`, `claude-statusline`, `codex` and `xai` declares exactly one credential name — `token` or `apiKey`, per the table. That is the name to re-declare when repointing a preset's key. `claude` and `kimi` try several locations in order, so they keep working whether or not you have set the CLI's home-directory variable. `codex` declares `token` and `accountId` from those same locations because its reset action sends both values. `xai` declares `apiKey` and `teamId`, because its balance route is per team and the team id lives in the url; the id is not a secret, but the credential map is the only thing that interpolates into a url, so it is stored like one. The remaining four declare none: `antigravity` and `github-copilot` read their credentials themselves, and the other two need no credential at all.
